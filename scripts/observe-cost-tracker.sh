#!/bin/bash
# observe-cost-tracker.sh — cast-observe PostToolUse cost tracking hook
...
if [ ! -f "$DB_PATH" ]; then
  DB_INIT="$(dirname "$0")/observe-db-init.sh"
  # Try script's own directory first, then installed location
  if [ ! -f "$DB_INIT" ]; then
...
try:
    ...
    # Insert agent_runs row only for Agent tool calls
